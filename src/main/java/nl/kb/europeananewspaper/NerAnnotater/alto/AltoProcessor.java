	...
	public static void handlePotentialAltoFile(URL potentialAltoFilename,
		...
		if ("text/xml".equalsIgnoreCase(mimeType)
			...
			try {
				...
				CRFClassifier<CoreLabel> classifier = (CRFClassifier<CoreLabel>) NERClassifiers.getCRFClassifierForLanguage(lang);
				
				List<List<CoreLabel>> coreMapElements = TextElementsExtractor.getCoreLabelElements(doc);
...
//				int lineNmbr=1;
				...
				int classified=0;
				for (List<CoreLabel> line: coreMapElements) {
...
//					System.out.println();
					...
					List<CoreLabel> classify = classifier.classify(line);
					
					for (CoreLabel label:classify) {
						
						totalNumberOfWords+=1;
						if (!label.get(AnswerAnnotation.class).equals("O")) {
							classified+=1;
							handler.addEntry(label.get(AltoStringID.class),label.word(),label.get(AnswerAnnotation.class));
						}
					}
				}
				
				System.out.println();
				System.out.println("Statistics: "+classified+" out of "+totalNumberOfWords+" ("+(new Double(classified)/new Double(totalNumberOfWords))+") classified");
